...
class AllTicketsListAdapter(private val context: Activity, private val dataSet:ArrayList<Ticket>,public val locationViewModel: LocationViewModel)
    : BaseAdapter() {
        ...
        if(dataSet[p0].serviceName!="NEMA TIKETA") {
            ...
            if (dataSet[p0].dateTimeForService != null && dataSet[p0].dateTimeForService != "") {
                ...
                view.findViewById<Button>(R.id.liveQueueButton).setOnClickListener {
                    context.findNavController(R.id.main_fragment_container).navigate(R.id.action_allTicketsFragment_to_liveQueueFragment)
                }
            }
